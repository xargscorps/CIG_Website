...
Netlify will run `npm run build` and publish the `dist` folder automatically.
...
1. Run `npm install` and `npm run build` on your computer.
...
Then run `npm run build` again before uploading.
